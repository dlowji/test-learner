...
def get_learner_pathways_associated_with_course(user, course_id):
    ...
    try:
        ...
        log.exception(
            ...
            f" linked with course: {course_id} from discovery. "
        )
...
def update_learner_pathway_progress(user_id, course_id):
    ...
    log.info(
        f"{PATHWAY_LOGS_IDENTIFIER}: Update pathways associated with course: {course_id} for user {user_id}"
    )
    pathways_linked_with_course = get_learner_pathways_associated_with_course(user, course_id)
...
def back_fill_learner_pathway_membership_and_progress(pathways):
    """Update the pathway progress and created membership for user if not already exist."""
    ...
    for page_number in paginator.page_range:
        ...
        for pathway in pathways:
            ...
            for enrollment in page.object_list:
                ...
                if not pathway_progress_exist and enrollment.course_id in pathway_courses:
...
def get_pathway_course_and_course_run_keys(step_courses, step_programs, courses):
    """Return all the course run keys linked with pathway step."""
    for course in step_courses:
        course_runs = course.get('course_runs') or []
        for course_run in course_runs:
            courses.append(course_run['key'])
    for program in step_programs:
        program_courses = program.get('courses') or []
        for p_course in program_courses:
            p_course_runs = p_course.get('course_runs') or []
            for p_course_run in p_course_runs:
                courses.append(p_course_run['key'])
    ...
    if user:
        ...
        for pathway_snapshot in pathway_snapshots:
            ...
            for step in pathway_steps:
                ...
                get_pathway_course_and_course_run_keys(step_courses, step_programs, courses)
            pathway_details['pathway-uuid'] = pathway_snapshot['uuid']
            pathway_details['courses'] = courses
